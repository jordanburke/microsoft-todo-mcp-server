...
=================================================================
    ...
    console.error(
      `Response received: ${JSON.stringify(data).substring(0, 200)}...`,
    );
    return data as T;
  } catch (error) {
        ...
        content: [
          {
            type: "text",
            text:
              "Not authenticated. Please run 'npx microsoft-todo-mcp-server setup' to authenticate with Microsoft.",
          },
        ],
      };
    }
        ...
        content: [
          {
            type: "text",
            text:
              `Authentication expired at ${expiryTime}. Will attempt to refresh when you call any API.${accountMessage}`,
          },
        ],
      };
    } else {
...
server.tool(
  ...
  "Get all task lists organized into logical folders/categories based on naming patterns, emoji prefixes, and sharing status. Provides a hierarchical view similar to folder organization.",
  {
    includeIds: z
      .boolean()
      .optional()
      .describe("Include list IDs in output (default: false)"),
    groupBy: z
      .enum(["category", "shared", "type"])
      .optional()
      .describe(
        "Grouping strategy - 'category' (default), 'shared', or 'type'",
      ),
  },
    ...
    try {
      ...
      const organizeLists = (
        ...
        lists.forEach((list) => {
          ...
          if (archiveMatch) {
            ...
            if (!organized["🚗 Travel & Rangeley"]) {
              organized["🚗 Travel & Rangeley"] = [];
            }
      ...
      sortedCategories.forEach((category) => {
        ...
        categoryLists.forEach((list, index) => {
          ...
          const metadata = [];
          if (list.wellknownListName === "defaultList") {
            metadata.push("Default");
          }
          if (list.wellknownListName === "flaggedEmails") {
            metadata.push("Flagged Emails");
          }
      ...
      // Add summary
      const totalLists = Object.values(organized).reduce(
        (sum, l) => sum + l.length,
        0,
      );
...
server.tool(
  ...
  "Get tasks from a specific Microsoft Todo list. These are the main todo items that can contain checklist items (subtasks).",
  {
    ...
    filter: z
      .string()
      .optional()
      .describe("OData $filter query (e.g., 'status eq \\'completed\\'')"),
    select: z
      .string()
      .optional()
      .describe(
        "Comma-separated list of properties to include (e.g., 'id,title,status')",
      ),
    orderby: z
      .string()
      .optional()
      .describe("Property to sort by (e.g., 'createdDateTime desc')"),
    ...
    count: z
      .boolean()
      .optional()
      .describe("Whether to include a count of tasks"),
  },
  async ({ listId, filter, select, orderby, top, skip, count }) => {
    try {
      ...
      const url = `${MS_GRAPH_BASE}/me/todo/lists/${listId}/tasks${
...
server.tool(
  ...
  "Create a new task in a specific Microsoft Todo list. A task is the main todo item that can have a title, description, due date, and other properties.",
  {
    ...
    body: z
      .string()
      .optional()
      .describe("Description or body content of the task"),
    dueDateTime: z
      .string()
      .optional()
      .describe("Due date in ISO format (e.g., 2023-12-31T23:59:59Z)"),
    startDateTime: z
      .string()
      .optional()
      .describe("Start date in ISO format (e.g., 2023-12-31T23:59:59Z)"),
    importance: z
    ...
    isReminderOn: z
      .boolean()
      .optional()
      .describe("Whether to enable reminder for this task"),
    reminderDateTime: z
      .string()
      .optional()
      .describe("Reminder date and time in ISO format"),
    status: z
      .enum([
        "notStarted",
        "inProgress",
...
server.tool(
  ...
  "Update an existing task in Microsoft Todo. Allows changing any properties of the task including title, due date, importance, etc.",
  {
    ...
    body: z
      .string()
      .optional()
      .describe("New description or body content of the task"),
    dueDateTime: z
      .string()
      .optional()
      .describe("New due date in ISO format (e.g., 2023-12-31T23:59:59Z)"),
    startDateTime: z
      .string()
      .optional()
      .describe("New start date in ISO format (e.g., 2023-12-31T23:59:59Z)"),
    importance: z
      .enum(["low", "normal", "high"])
      .optional()
      .describe("New task importance"),
    isReminderOn: z
      .boolean()
      .optional()
      .describe("Whether to enable reminder for this task"),
    reminderDateTime: z
      .string()
      .optional()
      .describe("New reminder date and time in ISO format"),
    status: z
      .enum([
        "notStarted",
        "inProgress",
          ...
          content: [
            {
              type: "text",
              text:
                "No properties provided for update. Please specify at least one property to change.",
            },
          ],
        };
      }
...
server.tool(
  ...
  "Create a new checklist item (subtask) for a task. Checklist items help break down a task into smaller, manageable steps.",
  {
    ...
    isChecked: z
      .boolean()
      .optional()
      .describe("Whether the item is checked off"),
  },
...
server.tool(
  ...
  "Update an existing checklist item (subtask). Allows changing the text content or completion status of the subtask.",
  {
    ...
    displayName: z
      .string()
      .optional()
      .describe("New text content of the checklist item"),
    isChecked: z
      .boolean()
      .optional()
      .describe("Whether the item is checked off"),
          ...
          content: [
            {
              type: "text",
              text:
                "No properties provided for update. Please specify either displayName or isChecked.",
            },
          ],
        };
      }
        ...
        content: [
          {
            type: "text",
            text:
              `Checklist item with ID: ${checklistItemId} was successfully deleted from task: ${taskId}`,
          },
        ],
      };
    } catch (error) {
...
server.tool(
  ...
  "Move completed tasks older than a specified number of days from one list to another (archive) list. Useful for cleaning up active lists while preserving historical tasks.",
  {
    sourceListId: z
      .string()
      .describe("ID of the source list to archive tasks from"),
    ...
    olderThanDays: z
      ...
      .default(90)
      .describe(
        "Archive tasks completed more than this many days ago (default: 90)",
      ),
    dryRun: z
      .boolean()
      .optional()
      .default(false)
      .describe(
        "If true, only preview what would be archived without making changes",
      ),
  },
  async ({ sourceListId, targetListId, olderThanDays, dryRun }) => {
    try {
      ...
      if (dryRun) {
        ...
        preview +=
          `Would archive ${tasksToArchive.length} tasks completed before ${cutoffDate.toLocaleDateString()}\n\n`;
...
server.tool(
  ...
  async ({ testType }) => {
    try {
          ...
          try {
            ...
            if (response && response.value) {
              ...
              if (response.value.length > 0 && response.value[0][expand]) {
                results += `Found data!\n`;
                results += JSON.stringify(response.value[0][expand], null, 2).substring(
                  0,
                  500,
                ) + "...\n";
              } else {
      ...
      if (testType === "all") {
        ...
        for (const endpoint of endpoints) {
          try {
            ...
            results += `✓ ${endpoint}: Found! Response: ${
              JSON.stringify(
                response,
              ).substring(0, 200)
            }...\n`;
      ...
      results += "\n" + "=".repeat(50) + "\n";
      results +=
        "Analysis complete. Check results above for any discovered properties or endpoints.";
